...
import { acceptUser } from '../../store/slices/usersManagement'
...
const AcceptUserModal: FC<InputProps & Props> = ({ isOpen, toggle, user }) => {
 // ===========================================================================
 // Selectors
 // ===========================================================================
   ...
   >
    <ModalBody className="newappointment__resultmodal-body">
     ...
     <Alert isOpen={open} className="clinity-alert" color={!error ? 'success' : 'danger'}>
      {!error ? '🎉 User Account  was successfuly accepted !' : '🤕 Sorry something went wrong !'}
     </Alert>
     <div>
      <AwesomeButton onClick={handleSubmit}>
       {loading ? <Spinner animation="border" /> : 'Accept User'}
      </AwesomeButton>
     </div>
    </ModalBody>
   </Modal>
